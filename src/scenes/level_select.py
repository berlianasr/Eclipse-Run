...
# --- Konstanta Warna Gaya Pixel Art ---
...
LEVEL_NUMBER_RENDER_SIZE = (220, 220)  # ukuran yang akan dirender untuk angka (lebih besar dari kotak)
LOCK_IMAGE_SIZE = (72, 72)
MAX_UPSCALE = 2.5  # allow images to be upscaled up to this factor

class LevelSelectScreen(BaseScreen):
    def __init__(self, manager, screen_width, screen_height):
        # --- PERBAIKAN SINTAKSIS DI SINI ---
        # Mengganti 'super().__init>' menjadi 'super().__init__()'
        ...
        # --- INISIALISASI TOMBOL KEMBALI (BACK) ---
    ...
    def _load_ui_images(self):
        ...
        # Load gambar "level" text (judul). Keep original and scale in draw
        level_text_path = os.path.join(IMAGE_DIR, "level.png")
        try:
            img = pygame.image.load(level_text_path).convert_alpha()
            # Keep original image and scale dynamically in draw() to avoid gepeng
    ...
    def _scale_preserve(self, image: pygame.Surface, max_size: tuple) -> pygame.Surface:
        ...
        # Calculate scale factor to fit inside the box while preserving aspect
        scale = min(max_w / w, max_h / h)
        # Allow upscaling but cap it to MAX_UPSCALE to avoid extreme pixelation
    ...
    def draw(self, surface):
        ...
        # --- Draw "level" text image above the boxes (larger, closer) ---
        # Scale the title dynamically larger while preserving aspect ratio,
        # then position it just above the level boxes so it's closer to options.
        if hasattr(self, 'level_text_image_orig') and self.level_text_image_orig:
            # Make the title even larger and allow taller maximum height
            ...
            # find top of boxes and position title lower (closer / slightly overlapping)
            if self.level_buttons:
                boxes_top = min(b.rect.top for b in self.level_buttons)
                # Positive offset moves title downward: place 8px into the area above boxes
        ...
        for i, btn in enumerate(self.level_buttons):
            ...
            # Hover detection: jika kursor di atas kotak, buat warna ungu meredup
            ...
            # --- KONTEN: DRAW ANGKA LEVEL DULU (Selalu) ---
            # Render angka level lebih besar dari kotak (menggunakan original dan scale_preserve)
            ...
            # --- DRAW GEMBOK (Hanya jika terkunci) ---
            if not btn.unlocked:
                # 1. Redupkan angka level dengan lapisan transparan
                s = pygame.Surface(btn.rect.size, pygame.SRCALPHA)
                # Warna hitam transparan (120 adalah opasitas/alpha)
                s.fill((0, 0, 0, 120))
                surface.blit(s, btn.rect.topleft)

                # 2. Gambar gembok (lock.png) di atas lapisan redup
                if self.lock_image:
